...
class DailyNotePlugin(Star):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        self.config = config
        self.db_path = os.path.join("data", "daily_note.db")
        self._init_db()
        ...
        for conv in conversations:
            try:
                await self._generate_note_for_conversation(
                    umo=conv.platform_id,
                    cid=conv.cid,
                    persona_id=conv.persona_id,
                )
            except Exception as e:
                logger.error(f"[daily_note] 会话 {conv.cid} 生成失败: {e}")
    ...
    async def _generate_note_for_conversation(
        ...
        if not provider_id:
            try:
                ...
                logger.warning("[daily_note] 无法获取默认 provider，跳过")
                return
        ...
        logger.info(f"[daily_note] 为 {umo} 生成日记完成: {title}")
    ...
    def _save_note(self, title: str, content: str, umo: str):
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                "INSERT INTO daily_notes (title, content, unified_msg_origin, created_at) VALUES (?, ?, ?, ?)",
    ...
    def _get_note_by_dynamic_index(self, umo: str, index: int) -> tuple | None:
    ...
    async def cmd_daily_note_gen(self, event: AstrMessageEvent):
        ...
        try:
            ...
            if notes:
                ...
                yield event.plain_result(f"日记生成成功：{latest_title}\n使用 /daily_note 1 查看内容")
            else:
                yield event.plain_result("日记生成流程完成，但未产生记录。")
        except Exception as e:
            logger.error(f"[daily_note] 手动生成失败: {e}")
